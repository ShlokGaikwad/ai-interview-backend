...
export async function startRealtimeInterviewer(roomName: string): Promise<void> {
  ...
  if (!interviewId) throw new Error('[Realtime] INTERVIEW_ID env var is required — agent must be spawned by the backend');

  const interviewDoc = await Interview.findById(interviewId).catch(() => null);
  if (!interviewDoc) throw new Error(`[Realtime] Interview not found: ${interviewId}`);

  const caseStudyId = interviewDoc?.caseStudyIds?.[0];
  if (!caseStudyId) throw new Error('[Realtime] No case study assigned to this interview — cannot start');
  ...
  try {
    ...
    console.error('[Realtime] Failed to connect to OpenAI — aborting:', (err as Error).message);
  ...
  const SILENCE_FIRST_NUDGE_MS = Number(process.env.REALTIME_SILENCE_FIRST_MS ?? 10000);
  const SILENCE_REPEAT_MS = Number(process.env.REALTIME_SILENCE_REPEAT_MS ?? 25000);
  ...
  const PROCESSING_WATCHDOG_MS = Number(process.env.REALTIME_PROCESSING_WATCHDOG_MS ?? 8000);
  ...
  function armProcessingWatchdog() {
    ...
    processingWatchdog = setTimeout(() => {
      ...
      if (state === 'processing') {
        console.warn('[Realtime] Response never started — forcing back to listening');
  ...
  function disarmProcessingWatchdog() {
    if (processingWatchdog) { clearTimeout(processingWatchdog); processingWatchdog = null; }
  }
  ...
  const SPEECH_WATCHDOG_MS = Number(process.env.REALTIME_SPEECH_WATCHDOG_MS ?? 18000);
  ...
  function armSpeechWatchdog() {
    ...
    speechWatchdog = setTimeout(() => {
      speechWatchdog = null;
      if (state === 'listening') {
        console.warn('[Realtime] Speech detection stuck (likely background noise) — clearing input buffer');
  ...
  if (interviewDoc) {
    try {
      const existingTranscript = await Transcript.findOne({ interviewId: interviewDoc._id });
      if (existingTranscript && existingTranscript.turns.length > 0) {
        ...
        turnCount = existingTranscript.turns[existingTranscript.turns.length - 1].turn;
        isRecovery = true;
        console.log(`[Realtime] Recovered ${existingTranscript.turns.length} turns from MongoDB`);
      }
    } catch (err) {
      console.warn('[Realtime] Could not load existing transcript:', (err as Error).message);
    }
  }
  ...
  const PLAYBACK_TAIL_COOLDOWN_MS = Number(process.env.REALTIME_PLAYBACK_COOLDOWN_MS ?? 250);
  ...
  function enqueueAudioDelta(base64Chunk: string): void {
    const combined = Buffer.concat([pcmResidual, Buffer.from(base64Chunk, 'base64')]);
    let offset = 0;
    while (offset + BYTES_PER_FRAME <= combined.length) {
      ...
      playQueue.push(new AudioFrame(data, SAMPLE_RATE, NUM_CHANNELS, SAMPLES_PER_CHANNEL));
  ...
  async function onAIResponseDone(responseText: string): Promise<void> {
    ...
    if (isFarewell(responseText)) {
      endingCall = true;
      console.log('[Realtime] Farewell detected — ending interview after playback');
      clearSilenceTimer();
      if (interviewDoc) {
  ...
  realtime.on('event', (event: RealtimeEvent) => {
    ...
    switch (event.type) {
      ...
      case 'conversation.item.input_audio_transcription.completed':
        ...
        if (event.transcript && event.transcript.trim()) {
          if (isNonEnglishNoise(event.transcript)) {
            console.log(`[Realtime] Ignored non-English/background transcript: "${event.transcript}"`);
      ...
      case 'response.output_audio_transcript.done':
        if (typeof event.transcript === 'string') currentResponseTranscript = event.transcript;
        break;
    ...
    void (async () => {
      for await (const frame of audioStream) {
        ...
        if (framesForwarded === 1) {
          console.log(`[Realtime] Forwarding candidate audio → OpenAI (${SAMPLE_RATE} Hz PCM16)`);
      ...
      console.log(`[Realtime] Candidate audio stream ended (${framesForwarded} frames total)`);
    })();
  }
  ...
  room.on(RoomEvent.ParticipantDisconnected, (participant: RemoteParticipant) => {
    ...
    console.log(`[Realtime] Candidate disconnected: ${participant.identity} — pausing`);
    clearSilenceTimer();
  ...
  room.on(RoomEvent.ParticipantConnected, (participant: RemoteParticipant) => {
    if (participant.identity.startsWith('observer-')) {
      console.log(`[Realtime] Observer joined — skipping greeting: ${participant.identity}`);
      return;
    }
