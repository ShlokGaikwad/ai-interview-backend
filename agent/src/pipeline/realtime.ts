...
export class RealtimeSession extends EventEmitter {
  ...
  connect(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        reject(new Error('[Realtime] Connection timed out after 15s (no session.updated)'));
        ws.terminate();
      }, 15000);
      ...
      ws.on('message', (raw: WebSocket.RawData) => {
        ...
        try {
          ...
          console.warn('[Realtime] Non-JSON message:', raw.toString().slice(0, 200));
          return;
        }
      ...
      ws.on('close', (code: number, reason: Buffer) => {
        console.log(`[Realtime] WebSocket closed (${code}) ${reason.toString()}`);
        ...
        if (!this.configured) {
          clearTimeout(timeout);
          reject(new Error(`[Realtime] Connection closed before ready (${code}): ${reason.toString() || 'no reason given'}`));
        }
      });
    });
  }
  ...
  private sendSessionUpdate(): void {
    ...
    const voice = this.config.voice ?? process.env.REALTIME_VOICE ?? 'cedar';
    const speed = Number(process.env.REALTIME_SPEED ?? this.config.speed ?? 0.9);
    // Higher threshold + longer trailing silence = brief backchannels ("yeah",
    // "mm-hm") don't get treated as a full turn and restart AI.
    const vadThreshold = Number(process.env.REALTIME_VAD_THRESHOLD ?? 0.6);
    const vadSilenceMs = Number(process.env.REALTIME_VAD_SILENCE_MS ?? this.config.silenceMs ?? 900);
    ...
    const vadMode = process.env.REALTIME_VAD_MODE ?? 'semantic';
    const turnDetection = vadMode === 'server'
      ? {
          type: 'server_vad',
      ...
      : {
          type: 'semantic_vad',
          eagerness: process.env.REALTIME_VAD_EAGERNESS ?? 'auto', // low|medium|high|auto
          create_response: false,
          interrupt_response: false,
        };
    ...
    console.log(`[Realtime] Sent session.update (voice: ${voice}, speed: ${speed}, vad: ${turnDetection.type})`);
  }

  private handleEvent(event: RealtimeEvent, onReady: () => void): void {
    switch (event.type) {
      case 'session.created':
        console.log('[Realtime] session.created — session id:', event.session?.id ?? '(unknown)');
        break;
  ...
  send(payload: unknown): void {
    if (this.ws?.readyState === WebSocket.OPEN) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      console.warn('[Realtime] Dropped send — socket not open:', (payload as any)?.type);
    }
  }
